# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 17:49:25 2019
...
with open('iris.data') as iris :
    x=iris[:,]                                                                        #为了后续的绘图方便只选取了前两个属性进行测试
    y=iris[:4]
    x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=1, train_size=0.6)
'''
def chartonum(s):
    Is = {b'Iris-setosa':0, b'Iris-versicolor':1, b'Iris-virginica':2}
    return Is[s]
fname = r'C:\Users\Karlleenings\Desktop\svmfortest\iris.data'
Data = np.loadtxt(fname,dtype=float,delimiter=',',converters={4:chartonum})
x = Data[:,1:5]
y = Data[:,4]
x_train, x_test, y_train, y_test = train_test_split(x, y, random_state=1, train_size=0.6)
#核心代码 gamma值越高越好，
clf1 = svm.SVC(C=0.6, kernel='rbf', gamma=10, decision_function_shape='ovr')            
...
#y_hat = clf1.predict(x_train,y_train)
#print(y_hat)
#show_accuracy(y_hat,y_train,'for train')
print (clf1.score(x_test,y_test))
#y_hat2 = clf1.predict(x_test,y_test)
#show_accuracy(y_hat2,y_test,'for test')
...
'''
#区域预测
x1_min, x1_max = x[:, 0].min(), x[:, 0].max()            # 第1列的范围
...
plt.xlabel(iris.feature_names[0]+':PETAL LENGTH', fontsize=8)  
plt.ylabel(iris.feature_names[1]+':PETAL WIDTH', fontsize=8)
plt.xlim(x1_min, x1_max)  
plt.ylim(x2_min, x2_max)
